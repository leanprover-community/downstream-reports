...
| `apply-fixes` | no | `true` | After the bump, run `hopscotch fix apply` so the PR carries the import repairs, not just the rev bump. Like the tool's default, applies **both** the failure-boundary `proposedFixes` (for an FKB bump, overlaid from the regression probe's published wide-range bisection — robust, offline migrations) **and** deprecated-import advisories (imports routing through a live `deprecated_module` shim that breaks at the upstream cleanup). Runs on any `query-type`. Set `no-advisories` to restrict it to the boundary repairs. **Best-effort:** if no fixes were recorded, the installed `hopscotch-version` lacks the `fix` subcommand, or the apply fails, the bump proceeds with the rev bump alone. Validated by the PR's own CI. |
| `no-advisories` | no | `false` | Pass `--no-advisories` to `hopscotch fix apply` (mirrors hopscotch's own flag; advisories apply by default), restricting it to the boundary `proposedFixes`. Set this to keep an LKG bump "mergeable as-is" — advisory rewrites aren't covered by the bump's green build. Advisories are read from the bump's **own** `results.json` (commit-specific: detected at the commit bumped to, not the published snapshot), so they're complete only when that bump built; `skip-build` finds only the tree-resolved subset. Worst case is incomplete, never wrong; partial migrations are skipped by hopscotch. No effect when `apply-fixes` is false. |
