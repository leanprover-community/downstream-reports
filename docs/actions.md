...
| `apply-fixes` | no | `true` | After the bump, run `hopscotch fix apply` so the PR carries the import repairs, not just the rev bump. By default applies **only** the failure-boundary `proposedFixes` — for an FKB bump, the boundary repair, overlaid from the regression probe's published wide-range bisection (robust, offline migrations); an LKG bump is green so it has none (no-op). Runs on any `query-type`. Set `apply-advisories` to also migrate advisories. **Best-effort:** if no fixes were recorded, the installed `hopscotch-version` lacks the `fix` subcommand, or the apply fails, the bump proceeds with the rev bump alone. Validated by the PR's own CI. |
| `apply-advisories` | no | `false` | Opt-in hygiene pass: also migrate deprecated-but-working imports (ones routing through a live `deprecated_module` shim that breaks at the upstream cleanup). **Off by default** — these imports build today, so the rewrites aren't covered by an LKG bump's green build and would undercut its "mergeable as-is" guarantee; enable for an explicit hygiene pass, ideally where the PR's CI runs (a GitHub App / PAT token). Advisories are read from the bump's **own** `results.json` (commit-specific: detected at the commit bumped to), so they're complete only when that bump built; `skip-build` finds only the tree-resolved subset; partial migrations are skipped by hopscotch. No effect when `apply-fixes` is false. |
...
| `fix-summary` | hopscotch's own `fix apply` output, verbatim, when it rewrote imports after the bump — empty when nothing was applied, the step was skipped, or no fixes were recorded. Pass to `open-bump-pr`'s `fix-summary` input to describe the applied migrations in the PR body using the tool's own words. |
