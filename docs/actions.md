...
| `apply-fixes` | no | `true` | After the bump, run `hopscotch fix apply` so the PR carries the fixes hopscotch recorded, not just the rev bump. Applies everything hopscotch proposes — the failure-boundary fixes (for an FKB bump, overlaid from the regression probe's published wide-range bisection; an LKG bump is green so it has none) and the deprecation advisories. Runs on any `query-type`. Set `no-advisories` to restrict it to the boundary fixes. **Best-effort:** if no fixes were recorded, the installed `hopscotch-version` lacks the `fix` subcommand, or the apply fails, the bump proceeds with the rev bump alone. Validated by the PR's own CI. |
