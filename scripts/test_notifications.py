...
class TestComputeAlertActions:
    ...
    def test_new_failure_produces_alert(self) -> None:
        ...
        assert actions[0].downstream == "physlib"
    ...
    def test_multiple_downstreams(self) -> None:
        ...
        assert {a.downstream for a in actions} == {"physlib", "other"}
    ...
    def test_action_uses_provided_stream_and_topic(self) -> None:
        ...
        assert actions[0].stream == "my-stream"
        assert actions[0].topic == "my-topic"
...
class TestCommitLinkWithTag:
    ...
    def test_tag_suppresses_commit_title(self) -> None:
        ...
        assert "feat: some feature" not in link
    ...
    def test_no_tag_falls_back_to_short_sha_and_title(self) -> None:
        ...
        assert f"[`{sha[:12]}`]" in link
        assert "feat: some feature" in link
...
class TestFormatNewFailureMessage:
    ...
    def test_includes_culprit_log_when_present(self) -> None:
        ...
        assert "error: type mismatch" in msg
        assert "```spoiler" in msg
    ...
    def test_target_commit_is_linked_sha(self) -> None:
        ...
        assert f"{_MATHLIB_COMMIT_URL}/{sha}" in msg
    ...
    def test_includes_commit_title_when_provided(self) -> None:
        ...
        assert "feat: add some feature" in msg
    ...
    def test_run_link_uses_downstream_validation_run_label(self) -> None:
        ...
        msg = format_new_failure_message(_make_record(), _RUN_URL)
        assert "Downstream validation run" in msg
    ...
    def test_downstream_commit_title_shown_when_provided(self) -> None:
        ...
        assert "chore: bump mathlib" in msg
    ...
    def test_tagged_commit_shows_tag_name(self) -> None:
        ...
        assert "[`v4.19.0`]" in msg
        assert f"[`{sha[:12]}`]" not in msg
...
class TestFormatRecoveredMessage:
    ...
    def test_uses_previous_known_bad_label(self) -> None:
        ...
        assert "Previous known-bad" in msg
    ...
    def test_release_tag_shown_when_present(self) -> None:
        ...
        assert "v4.13.0" in msg
        assert f"{_MATHLIB_RELEASE_URL}/v4.13.0" in msg
        assert "Last compatible release" in msg

    def test_release_tag_absent_when_none(self) -> None:
        """Scenario: no last_good_release → release line is omitted."""
        msg = format_new_failure_message(_make_record(), _RUN_URL)
        assert "Last compatible release" not in msg
        assert _MATHLIB_RELEASE_URL not in msg
    ...
    def test_recovered_release_tag_shown_when_present(self) -> None:
        ...
        assert "v4.13.0" in msg
        assert f"{_MATHLIB_RELEASE_URL}/v4.13.0" in msg
        assert "Compatible with release" in msg
    ...
    def test_recovered_release_tag_absent_when_none(self) -> None:
        ...
        assert "Compatible with release" not in msg
        assert _MATHLIB_RELEASE_URL not in msg
...
class TestAlertableStates:
    ...
    def test_contains_new_failure_and_recovered(self) -> None:
        ...
        assert ALERTABLE_STATES == {"new_failure", "recovered"}
...
class TestFormatSummaryMessage:
    ...
    def test_renders_table_inside_spoiler(self) -> None:
        ...
        assert "```spoiler" in msg
        assert "| Downstream | Status | Good release | First Bad | Safe commits |" in msg
        assert "Target" not in msg
        assert "Last Good" not in msg
    ...
    def test_includes_all_downstreams_as_short_name_links(self) -> None:
        ...
        assert "[Zeta](https://github.com/owner/Zeta)" in msg
        assert "[alpha](https://github.com/owner/alpha)" in msg
        assert "[Beta](https://github.com/owner/Beta)" in msg
        # case-insensitive: alpha < Beta < Zeta
        assert msg.index("alpha") < msg.index("Beta")
        assert msg.index("Beta") < msg.index("Zeta")
    ...
    def test_present_first_bad_is_linkified(self) -> None:
        ...
        assert "deadbeef1234" in msg
        assert f"{_MATHLIB_COMMIT_URL}/deadbeef1234" in msg
    ...
    def test_includes_run_metadata(self) -> None:
        ...
        assert "12345" in msg
        assert _SUMMARY_RUN_META["run_url"] in msg
    ...
    def test_counts_appear_before_spoiler(self) -> None:
        ...
        msg = format_summary_message(_SUMMARY_RUN_META, rows)
        assert "2 compatible" in msg
        assert "1 incompatible" in msg
        assert "1 errors" in msg
        # Counts should appear before the spoiler block
        assert msg.index("compatible") < msg.index("```spoiler")
...
class TestFormatErrorNoticeMessage:
    ...
    def test_singular_noun(self) -> None:
        ...
        msg = format_error_notice_message(1, _RUN_URL)
        assert "1 build " in msg
        assert "builds" not in msg
...
class TestFormatOndemandFailureMessage:
    ...
    def test_incompatibility_headline(self) -> None:
        ...
        msg = format_ondemand_failure_message(_make_record(), _RUN_URL)
        assert "incompatible with the targeted Mathlib revision" in msg
...
class TestFormatOndemandCompatibleMessage:
    ...
    def test_compatibility_headline(self) -> None:
        ...
        assert "compatible with the targeted Mathlib revision" in msg
...
class TestComputeAlertActionsWorkflow:
    ...
    def test_ondemand_failure_uses_incompatibility_language(self) -> None:
        ...
        assert len(actions) == 1
        assert "incompatible with the targeted Mathlib revision" in actions[0].content
        assert "New regression" not in actions[0].content
    ...
    def test_ondemand_recovered_uses_compatibility_language(self) -> None:
        ...
        actions = compute_alert_actions(records, _RUN_URL, _STREAM, _TOPIC, workflow="ondemand")
        assert len(actions) == 1
        assert "compatible with the targeted Mathlib revision" in actions[0].content
        assert "recovered" not in actions[0].content
    ...
    def test_regression_failure_uses_regression_language(self) -> None:
        ...
        assert len(actions) == 1
        assert "New regression detected" in actions[0].content
    ...
    def test_ondemand_reports_all_non_error_states(self) -> None:
        ...
        assert [a.downstream for a in actions] == ["physlib", "b", "c"]
    ...
    def test_ondemand_includes_skipped_downstreams(self) -> None:
        ...
        assert len(actions) == 2
        assert actions[1].downstream == "skipped-ds"
        assert "was not retested" in actions[1].content


class TestFormatOndemandSkippedMessage:
    ...
    def test_compatible_outcome_label(self) -> None:
        ...
        assert "incompatible" not in msg
    ...
    def test_includes_previous_job_url(self) -> None:
        ...
        msg = format_ondemand_skipped_message(record, _RUN_URL)
        assert "https://example.com/job/42" in msg
    ...
    def test_skipped_headline(self) -> None:
        ...
        msg = format_ondemand_skipped_message(record, _RUN_URL)
        assert "was not retested" in msg
