...
| `fail_fast` | Set `false` to turn off `lake build --fail-fast` in the bisect builds. The default is `true`. The flag applies only to a toolchain that accepts it (v4.35.0-rc1 or later). |
